...
def sync_lab(
        ...
        appwrite_json: str = typer.Option("appwrite.json", help="The appwrite.json file to sync."),
        resource: SyncType = typer.Option(SyncType.ALL, show_default=True, help="The resource to sync.")):
    """
    ...
    with console.status(f"Syncing lab '{name}'...", spinner="dots") as status:
        labs.sync_with_appwrite_config(name=name, appwrite_json=appwrite_json, sync_type=resource)